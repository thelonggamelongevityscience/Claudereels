...
export const Scene1CholesterolHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const lines = ['YOUR DOCTOR', 'IS CHECKING', 'THE WRONG', 'CHOLESTEROL', 'NUMBER.'];
    ...
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {lines.map((text, i) => (
        <div key={i} style={{ overflow: 'hidden', marginBottom: 2 }}>
          <div style={{ ...mk((i + 1) * 7), fontFamily: FONTS.barlow, fontWeight: 900, fontSize: 120, lineHeight: 0.92, textTransform: 'uppercase', color: i === 4 ? COLORS.red : COLORS.white }}>
            {text}
          </div>
        </div>
      ))}
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 30, color: COLORS.greige, lineHeight: 1.4 }}>
        ...
        <span style={{ color: COLORS.white, fontStyle: 'normal', fontFamily: FONTS.barlow, fontWeight: 700, fontSize: 26 }}>
          Here is what actually predicts your risk.
        </span>
      </div>
