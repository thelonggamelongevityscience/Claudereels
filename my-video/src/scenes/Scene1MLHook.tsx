...
export const Scene1MLHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const lineW   = interpolate(frame, [58, 78], [0, 100], { extrapolateRight: 'clamp' });
  const subOp   = interpolate(frame, [68, 84], [0, 1], { extrapolateRight: 'clamp' });
  const subY    = interpolate(frame, [68, 84], [16, 0], { extrapolateRight: 'clamp' });

  const hlLines = [
    { text: 'YOUR FIRST', op: h1Op, y: h1Y },
    { text: '30 MINUTES', op: h2Op, y: h2Y },
    { text: 'AWAKE',      op: h3Op, y: h3Y },
    { text: 'DECIDE',     op: h4Op, y: h4Y },
    { text: "TONIGHT'S",  op: h5Op, y: h5Y },
    { text: 'SLEEP.',     op: h6Op, y: h6Y },
  ];

  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      <GlowBg color="gold" opacity={0.1} />
      <GridOverlay />

      <div style={{ opacity: emojiOp, fontSize: 96, lineHeight: 1, marginBottom: 20, filter: `drop-shadow(0 0 32px ${COLORS.gold}99)` }}>☀️</div>
      ...
      <div style={{ width: `${lineW}%`, height: 3, backgroundColor: COLORS.gold, margin: '18px 0 16px' }} />
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 32, color: COLORS.greige, lineHeight: 1.45 }}>
        Most people wreck it before they leave the bedroom.<br />
        <strong style={{ color: COLORS.white, fontStyle: 'normal' }}>Here's what's actually happening.</strong>
      </div>
