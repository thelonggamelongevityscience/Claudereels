...
const HormoneCard: React.FC<{ name: string; role: string; body: string }> = ({ name, role, body }) => (
  <div style={{
...
export const Scene3HormonesTostThyroid: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {(['THE OTHER TWO', 'MOST PEOPLE', 'NEVER TEST.'] as const).map((text, i) => (
      ...
      <div style={{ opacity: card1Op, transform: `translateY(${card1Y}px)` }}>
        <HormoneCard
          name="Testosterone"
          role="The Drive Hormone"
          body="Muscle synthesis, libido, motivation, cognitive focus, and bone density. Declining in men 10 years earlier than previous generations."
        />
      </div>
      ...
      <div style={{ opacity: card2Op, transform: `translateY(${card2Y}px)` }}>
        <HormoneCard
          name="Thyroid"
          role="The Metabolic Thermostat"
          body="Sets the speed of every cellular process. Subclinical hypothyroidism affects 1 in 5 people and is missed by standard TSH testing."
        />
      </div>
