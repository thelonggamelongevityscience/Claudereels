...
export const Scene5LAExampleSept: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      <div style={{ opacity: boxOp, transform: `translateY(${boxY}px)`, border: `2px solid ${COLORS.gold}`, backgroundColor: 'rgba(255,209,102,0.06)', padding: '36px 40px', marginBottom: 32 }}>
        ...
        <div style={{ fontFamily: FONTS.barlow, fontWeight: 900, fontSize: 96, color: COLORS.gold, textTransform: 'uppercase', letterSpacing: '-0.02em', lineHeight: 0.9 }}>CHAOTIC<br />GOOD</div>
      </div>
      ...
      <div style={{ opacity: descOp, transform: `translateY(${descY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 30, color: COLORS.greige, lineHeight: 1.5 }}>
        High potential, inconsistent execution.<br />
        <strong style={{ color: COLORS.white }}>Your biggest lever: consistency, not intensity.</strong>
      </div>
