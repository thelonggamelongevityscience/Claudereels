...
export const Scene7IRLoopHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const hlLines = [
    { text: 'SEND THIS TO', op: hl1Op, y: hl1Y },
    { text: 'SOMEONE WHO', op: hl2Op, y: hl2Y },
    { text: 'COMPLAINS OF', op: hl3Op, y: hl3Y },
    { text: 'FATIGUE OR BRAIN FOG.', op: hl4Op, y: hl4Y },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {hlLines.map((l, i) => (
        <div key={i} style={{ opacity: l.op, transform: `translateY(${l.y}px)`, fontFamily: FONTS.barlow, fontWeight: 900, fontSize: i === 3 ? 80 : 108, lineHeight: 1.0, textTransform: 'uppercase', color: COLORS.white, marginBottom: 6 }}>
          {l.text}
        </div>
      ))}
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 32, color: COLORS.greige, lineHeight: 1.5 }}>
        Those are not random symptoms.<br />
        <strong style={{ color: COLORS.orange, fontStyle: 'normal' }}>That is the fire. Now they know.</strong>
      </div>
