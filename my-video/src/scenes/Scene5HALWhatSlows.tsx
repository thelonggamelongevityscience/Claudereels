...
export const Scene5HALWhatSlows: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const items = [
    { label: 'Exercise', body: 'is the single intervention shown to positively influence the most hallmarks simultaneously.', op: li1Op, x: li1X },
    { label: 'Fasting and caloric moderation', body: 'improve nutrient sensing and trigger autophagy.', op: li2Op, x: li2X },
    { label: 'Quality sleep', body: 'supports cellular repair processes across nearly every hallmark.', op: li3Op, x: li3X },
    { label: 'Managing chronic stress', body: 'reduces the inflammatory signalling that accelerates all twelve.', op: li4Op, x: li4X },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      <div style={{ opacity: hlOp, transform: `translateY(${hlY}px)`, fontFamily: FONTS.barlow, fontWeight: 800, fontSize: 68, lineHeight: 0.92, textTransform: 'uppercase', color: COLORS.green, marginBottom: 28 }}>
        THE INTERVENTIONS<br />THAT MOVE MULTIPLE<br />HALLMARKS AT ONCE.
      </div>
