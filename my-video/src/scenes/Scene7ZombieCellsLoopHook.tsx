...
export const Scene7ZombieCellsLoopHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {(['DID YOU KNOW', 'ZOMBIE CELLS', 'WERE REAL?'] as const).map((text, i) => (
        <div key={i} style={{ overflow: 'hidden', marginBottom: 4 }}>
          <div style={{ ...mk((i + 1) * 8), fontFamily: FONTS.barlow, fontWeight: 900, fontSize: 130, lineHeight: 1.0, textTransform: 'uppercase', color: i === 1 ? COLORS.red : COLORS.white }}>
            {text}
          </div>
        </div>
      ))}

      <div style={{ width: `${lineWidth}%`, height: 3, backgroundColor: COLORS.red, margin: '24px 0' }} />
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 32, color: COLORS.greige, lineHeight: 1.4 }}>
        Most people have never heard of senescence.<br />
        <strong style={{ color: COLORS.white, fontStyle: 'normal' }}>Drop a 🧟 below if this blew your mind. 👇</strong>
      </div>
