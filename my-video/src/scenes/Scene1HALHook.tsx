...
export const Scene1HALHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const hlLines = [
    { text: 'THE',      op: h1Op, y: h1Y },
    { text: 'FRAMEWORK', op: h2Op, y: h2Y },
    { text: 'BEHIND',   op: h3Op, y: h3Y },
    { text: 'EVERY',    op: h4Op, y: h4Y },
    { text: 'POST',     op: h5Op, y: h5Y },
    { text: "WE'VE MADE.", op: h6Op, y: h6Y },
  ];

  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      <GlowBg color="indigo" opacity={0.1} />
      <GridOverlay />

      <div style={{ opacity: emojiOp, fontSize: 96, lineHeight: 1, marginBottom: 20, filter: `drop-shadow(0 0 32px ${COLORS.indigo}99)` }}>🧬</div>
      ...
      <div style={{ width: `${lineW}%`, height: 3, backgroundColor: COLORS.indigo, margin: '18px 0 16px' }} />
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 32, color: COLORS.greige, lineHeight: 1.45 }}>
        Twelve mechanisms. One unifying map.<br />
        <strong style={{ color: COLORS.white }}>This is the science underneath all of it.</strong>
      </div>
