...
export const Scene5BloodSugarFix: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const bullets: { delay: number; color: 'green' | 'gold'; label: string; text: string }[] = [
    { delay: 28, color: 'green', label: 'Eat in order:', text: 'fibre and protein first, carbohydrates last — reduces spike by up to 73%' },
    { delay: 40, color: 'green', label: '10 minute walk after meals:', text: 'muscle glucose uptake drops blood sugar faster than any drug' },
    { delay: 52, color: 'green', label: 'Apple cider vinegar:', text: '1 tbsp before a meal blunts the glucose response significantly' },
    { delay: 64, color: 'gold', label: 'Never eat carbs alone:', text: 'always pair with fat or protein to slow absorption and flatten the curve' },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {(['HOW TO FLATTEN', 'YOUR GLUCOSE', 'CURVE.'] as const).map((text, i) => (
