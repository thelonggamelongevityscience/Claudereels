...
export const Scene4MetabolicDiseases: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const bullets: { delay: number; label: string; text: string }[] = [
    { delay: 28, label: 'Type 2 diabetes:', text: 'the end stage of insulin resistance — preventable at every stage before diagnosis' },
    { delay: 40, label: 'Cardiovascular disease:', text: 'metabolic dysfunction drives the inflammatory environment that builds arterial plaque' },
    { delay: 52, label: 'Alzheimer\'s:', text: 'now called Type 3 diabetes by researchers — insulin resistance in the brain impairs neuronal function' },
    { delay: 64, label: 'Cancer:', text: 'the high-glucose, high-insulin environment accelerates tumour growth and impairs immune surveillance' },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {(['EVERY MAJOR', 'DISEASE OF AGEING', 'STARTS HERE.'] as const).map((text, i) => (
