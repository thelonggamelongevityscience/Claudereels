...
export const Scene5MetabolicTests: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const bullets: { delay: number; label: string; text: string }[] = [
    { delay: 28, label: 'Fasting insulin:', text: 'the earliest metabolic warning signal — almost nobody tests it' },
    { delay: 40, label: 'HbA1c:', text: '3-month average glucose — below 5.4% is optimal, not just below 5.7%' },
    { delay: 52, label: 'Triglyceride to HDL ratio:', text: 'below 1.5 is optimal — the strongest non-invasive insulin resistance marker' },
    { delay: 64, label: 'ApoB:', text: 'the actual cardiovascular risk marker your standard panel is not measuring' },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      <div style={{ opacity: tagOp, transform: `translateY(${tagY}px)`, fontFamily: FONTS.barlow, fontWeight: 700, fontSize: 22, letterSpacing: '0.25em', textTransform: 'uppercase', color: COLORS.green, marginBottom: 20 }}>
        THE FIVE TESTS TO RUN NOW
      </div>

      {(['HOW TO KNOW', 'WHERE YOU', 'ACTUALLY STAND.'] as const).map((text, i) => (
