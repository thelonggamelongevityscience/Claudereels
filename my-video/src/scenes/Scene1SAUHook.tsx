...
export const Scene1SAUHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const hlLines = [
    { text: 'THE FINNISH', op: h1Op, y: h1Y },
    { text: 'HABIT LINKED', op: h2Op, y: h2Y },
    { text: 'TO LIVING',   op: h3Op, y: h3Y },
    { text: 'LONGER.',     op: h4Op, y: h4Y },
  ];

  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      <GlowBg color="ember" opacity={0.1} />
      <GridOverlay />

      <div style={{ opacity: emojiOp, fontSize: 96, lineHeight: 1, marginBottom: 20, filter: `drop-shadow(0 0 32px ${COLORS.ember}99)` }}>🧖</div>
      ...
      <div style={{ width: `${lineW}%`, height: 3, backgroundColor: COLORS.ember, margin: '18px 0 16px' }} />
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 32, color: COLORS.greige, lineHeight: 1.45 }}>
        It's not a supplement. It's not a diet.<br />
        <strong style={{ color: COLORS.white }}>It's heat.</strong>
      </div>
