...
export const Scene6CholesterolLoopHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const lines = ['HAS YOUR', 'DOCTOR EVER', 'TESTED YOUR', 'APOB OR LP(A)?'];
    ...
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {lines.map((text, i) => (
        <div key={i} style={{ overflow: 'hidden', marginBottom: 4 }}>
          <div style={{ ...mk((i + 1) * 8), fontFamily: FONTS.barlow, fontWeight: 900, fontSize: 110, lineHeight: 0.92, textTransform: 'uppercase', color: i === 3 ? COLORS.red : COLORS.white }}>
            {text}
          </div>
        </div>
      ))}
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 28, color: COLORS.greige, lineHeight: 1.45 }}>
        Most have not. These are the numbers that actually matter.{' '}
        <span style={{ color: COLORS.white, fontStyle: 'normal', fontFamily: FONTS.barlow, fontWeight: 700, fontSize: 24 }}>
          Comment LABS below.
        </span>
      </div>
