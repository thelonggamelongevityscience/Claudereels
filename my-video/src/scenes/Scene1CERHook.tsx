...
export const Scene1CERHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: BG, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 40%, rgba(0,229,255,0.10) 0%, transparent 65%)`, pointerEvents: 'none' }} />
      <GridOverlay />

      <div style={{ opacity: emojiOp, transform: `translateY(${emojiY}px)`, fontSize: 120, lineHeight: 1, marginBottom: 24, filter: `drop-shadow(0 0 40px rgba(0,229,255,0.6))` }}>
        🧊
      </div>
      ...
      <div style={{ opacity: hlOp, transform: `translateY(${hlY}px)`, fontFamily: FONTS.barlow, fontWeight: 900, fontSize: 86, lineHeight: 0.9, textTransform: 'uppercase', color: COLORS.white, marginBottom: 32 }}>
        COLD PLUNGES<br />ARE EVERYWHERE.<br />HERE'S WHAT'S<br />ACTUALLY PROVEN.
      </div>
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 34, color: COLORS.greige, lineHeight: 1.4 }}>
        Five common claims. <strong style={{ color: COLORS.white }}>Rated.</strong>
      </div>
