...
export const Scene1GutAnxietyHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      <GlowBg color="green" opacity={0.14} />
      <GridOverlay />
      ...
      {(['YOUR GUT', 'IS MAKING', 'YOU', 'ANXIOUS.'] as const).map((text, i) => (
        <div key={i} style={{ overflow: 'hidden', marginBottom: 4 }}>
          <div style={{ ...mk((i + 1) * 8), fontFamily: FONTS.barlow, fontWeight: 900, fontSize: 148, lineHeight: 1.0, textTransform: 'uppercase', color: i === 3 ? COLORS.green : COLORS.white }}>
            {text}
          </div>
        </div>
      ))}

      <div style={{ width: `${lineWidth}%`, height: 3, backgroundColor: COLORS.green, margin: '20px 0 24px' }} />
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 34, color: COLORS.greige, lineHeight: 1.4 }}>
        ...
        <strong style={{ color: COLORS.white, fontStyle: 'normal' }}>Your gut bacteria are running your mood.</strong>
      </div>
