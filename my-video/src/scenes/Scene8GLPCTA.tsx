...
export const Scene8GLPCTA: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const box2Op  = interpolate(frame, [22, 36], [0, 1], { extrapolateRight: 'clamp' });
  const box2Y   = interpolate(frame, [22, 36], [12, 0], { extrapolateRight: 'clamp' });
  const hashOp  = interpolate(frame, [32, 46], [0, 1], { extrapolateRight: 'clamp' });
  const hashY   = interpolate(frame, [32, 46], [10, 0], { extrapolateRight: 'clamp' });
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: '#07080c', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      <div style={{ opacity: box1Op, transform: `translateY(${box1Y}px)`, border: `1.5px solid ${STEEL}`, padding: '22px 28px', marginBottom: 16 }}>
        <div style={{ fontFamily: FONTS.barlow, fontWeight: 700, fontSize: 32, color: STEEL, letterSpacing: '0.1em', textTransform: 'uppercase' }}>→ Save this for the next debate</div>
      </div>
      ...
      <div style={{ opacity: hashOp, transform: `translateY(${hashY}px)`, fontFamily: FONTS.mono, fontSize: 20, color: 'rgba(91,141,239,0.6)', lineHeight: 1.8 }}>
        #glp1 #ozempic #longevity #metabolichealth #weightloss
      </div>
