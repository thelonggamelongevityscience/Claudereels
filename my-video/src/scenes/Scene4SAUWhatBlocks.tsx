...
export const Scene4SAUWhatBlocks: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const items = [
    { label: 'Occasional use only:', body: 'one session a month will not replicate the dose-response seen in the research.', op: li1Op, x: li1X },
    { label: 'Sessions too short:', body: 'the studies used roughly 15 to 20 minute sessions, not quick 5-minute dips.', op: li2Op, x: li2X },
    { label: 'Skipping hydration:', body: 'heat stress without fluid replacement blunts the benefit and adds unnecessary strain.', op: li3Op, x: li3X },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      <div style={{ opacity: hlOp, transform: `translateY(${hlY}px)`, fontFamily: FONTS.barlow, fontWeight: 800, fontSize: 72, lineHeight: 0.92, textTransform: 'uppercase', color: COLORS.white, marginBottom: 28 }}>
        WHY MOST<br />PEOPLE NEVER<br />GET THE DOSE.
      </div>

      {items.map((item, i) => (
        <div key={i} style={{ opacity: item.op, transform: `translateX(${item.x}px)`, display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 24 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: COLORS.red, flexShrink: 0, marginTop: 8 }} />
          <div style={{ fontFamily: FONTS.mono, fontSize: 23, color: COLORS.greige, lineHeight: 1.5 }}>
