...
export const Scene5LiverProtocol: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const bullets: { delay: number; label: string; text: string }[] = [
    { delay: 28, label: 'Remove fructose:', text: 'no fruit juice, no HFCS — fructose is processed exclusively by the liver and converts directly to fat' },
    { delay: 40, label: 'Coffee — seriously:', text: '2 to 3 cups daily reduces liver fibrosis risk by up to 40% in multiple large studies' },
    { delay: 52, label: 'Zone 2 exercise:', text: 'aerobic training directly reduces hepatic fat faster than dietary intervention alone' },
    { delay: 64, label: 'Test GGT and ALT:', text: 'add these to your annual panel — they give the earliest warning signal before damage becomes serious' },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      {(['HOW TO GIVE', 'YOUR LIVER A', 'FIGHTING CHANCE.'] as const).map((text, i) => (
      ...
      {bullets.map((b, i) => {
        ...
        return (
          <div key={i} style={{ opacity: op, transform: `translateX(${x}px)`, marginBottom: 22 }}>
            <BulletItem color="green" label={b.label} text={b.text} fontSize={21} />
          </div>
        );
      })}
