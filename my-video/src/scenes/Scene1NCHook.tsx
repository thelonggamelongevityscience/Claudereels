...
export const Scene1NCHook: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  const hlLines = [
    { text: 'PASSED OUT',  op: h1Op, y: h1Y },
    { text: 'IS NOT THE',  op: h2Op, y: h2Y },
    { text: 'SAME AS',     op: h3Op, y: h3Y },
    { text: 'ASLEEP.',     op: h4Op, y: h4Y },
  ];
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      ...
      <div style={{ opacity: emojiOp, fontSize: 96, lineHeight: 1, marginBottom: 20, filter: `drop-shadow(0 0 32px ${COLORS.red}99)` }}>🍷</div>
      ...
      <div style={{ width: `${lineW}%`, height: 3, backgroundColor: COLORS.red, margin: '18px 0 16px' }} />
      ...
      <div style={{ opacity: subOp, transform: `translateY(${subY}px)`, fontFamily: FONTS.playfair, fontStyle: 'italic', fontSize: 32, color: COLORS.greige, lineHeight: 1.45 }}>
        That nightcap is stealing something you don't get back.<br />
        <strong style={{ color: COLORS.white, fontStyle: 'normal' }}>Here's what alcohol actually does to your sleep.</strong>
      </div>
