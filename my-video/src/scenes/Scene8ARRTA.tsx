...
export const Scene8ARRTA: React.FC<Props> = ({ frame, captionChunks }) => {
  ...
  return (
    <div style={{ width: 1080, height: 1920, backgroundColor: BG, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 72px' }}>
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 50% 40%, rgba(168,85,247,0.07) 0%, transparent 65%)`, pointerEvents: 'none' }} />
      <GridOverlay />
      ...
      <div style={{ opacity: logoOp, transform: `translateY(${logoY}px)`, marginBottom: 36 }}>
        ...
        <div style={{ height: 2, backgroundColor: VIOLET, marginTop: 16, marginBottom: 12, width: '100%' }} />
        <div style={{ fontFamily: FONTS.mono, fontSize: 20, fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: VIOLET }}>Longevity Science</div>
      </div>

      <div style={{ opacity: box1Op, transform: `translateY(${box1Y}px)`, border: `1.5px solid ${VIOLET}`, padding: '22px 28px', marginBottom: 16 }}>
        <div style={{ fontFamily: FONTS.barlow, fontWeight: 700, fontSize: 32, color: VIOLET, letterSpacing: '0.1em', textTransform: 'uppercase' }}>→ Save this, it's science class</div>
      </div>
      ...
      <div style={{ opacity: hashOp, transform: `translateY(${hashY}px)`, fontFamily: FONTS.mono, fontSize: 20, color: `rgba(168,85,247,0.6)`, lineHeight: 1.8 }}>
        #autophagy #cellularhealth #longevity<br />#fasting #science
      </div>
