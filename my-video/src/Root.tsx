...
import { BedroomTempReel } from './BedroomTempReel';
import { PostMealWalkReel } from './PostMealWalkReel';
import { SocialConnectionReel } from './SocialConnectionReel';
import { AutophagyRemakeReel } from './AutophagyRemakeReel';
import { ColdExposureReel } from './ColdExposureReel';
import { VitaminDReel } from './VitaminDReel';
import { TRIIMReel } from './TRIIMReel';
import { NADReel } from './NADReel';
import { RapamycinReel } from './RapamycinReel';
import { MovementTypeReel } from './MovementTypeReel';
import { FPS, WIDTH, HEIGHT, ZOMBIE_CELLS_TOTAL_FRAMES, GUT_ANXIETY_TOTAL_FRAMES, BLOOD_SUGAR_TOTAL_FRAMES, HORMONES_TOTAL_FRAMES, METABOLIC_TOTAL_FRAMES, LIVER_TOTAL_FRAMES, CHOLESTEROL_TOTAL_FRAMES, BELLY_FAT_TOTAL_FRAMES, ZCR_TOTAL_FRAMES, IR_TOTAL_FRAMES, AR_TOTAL_FRAMES, ML_TOTAL_FRAMES, NC_TOTAL_FRAMES, MEL_TOTAL_FRAMES, VO2_TOTAL_FRAMES, FIBER_TOTAL_FRAMES, GRIP_TOTAL_FRAMES, HALLMARKS_TOTAL_FRAMES, SAUNA_TOTAL_FRAMES, BONE_TOTAL_FRAMES, HSCRP_TOTAL_FRAMES, LA_TOTAL_FRAMES, GLP_TOTAL_FRAMES, C15_TOTAL_FRAMES, BT_TOTAL_FRAMES, PMW_TOTAL_FRAMES, SC_TOTAL_FRAMES, ARR_TOTAL_FRAMES, CE_TOTAL_FRAMES, VD_TOTAL_FRAMES, TRIIM_TOTAL_FRAMES, NAD_TOTAL_FRAMES, RAP_TOTAL_FRAMES, MOV_TOTAL_FRAMES } from './constants';
...
export const RemotionRoot: React.FC = () => {
  return (
    <>
      ...
      <Composition
        id="BedroomTempReel"
        component={BedroomTempReel}
        durationInFrames={BT_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="PostMealWalkReel"
        component={PostMealWalkReel}
        durationInFrames={PMW_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="SocialConnectionReel"
        component={SocialConnectionReel}
        durationInFrames={SC_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="AutophagyRemakeReel"
        component={AutophagyRemakeReel}
        durationInFrames={ARR_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      ...
      <Composition
        id="VitaminDReel"
        component={VitaminDReel}
        durationInFrames={VD_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="TRIIMReel"
        component={TRIIMReel}
        durationInFrames={TRIIM_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="NADReel"
        component={NADReel}
        durationInFrames={NAD_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="RapamycinReel"
        component={RapamycinReel}
        durationInFrames={RAP_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
      <Composition
        id="MovementTypeReel"
        component={MovementTypeReel}
        durationInFrames={MOV_TOTAL_FRAMES}
        fps={FPS}
        width={WIDTH}
        height={HEIGHT}
      />
